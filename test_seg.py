...
def evaluate(args, model, img, device):
    ...
    '''for i, imgName in tqdm(enumerate(image_list)):
        ...
        #name = '{}/{}'.format("seg_results", name.replace(img_extn, 'png'))
        name = 'seg_results'.format("png")
        img_out.save("/home/aditya/ESPNetv2/seg_results/" +
                     "hello"+[i]+".png", "PNG")
    '''#w, h = img.size
    h, w, _ = img.shape
    img = data_transform(img, im_size)
...
if __name__ == '__main__':
    ...
    # set-up results path
    '''if image_list:
        args.savedir = '{}_{}_{}/results'.format(
            'results', image_list, args.split)
    elif args.dataset == 'pascal':
        args.savedir = '{}_{}/results/VOC2012/Segmentation/comp6_{}_cls'.format(
            'results', args.dataset, args.split)
    else:
        print_error_message(
            '{} dataset not yet supported'.format(args.dataset))
    ...
    # 'results', image_list, args.split)
    os.mkdir("/home/aditya/ESPNetv2/seg_results")
